...
class TCT(nn.Module):
    ...
    def forward(self, x):
        ...
        return self.transformer(x)                   # (batch, time, d_model)
...
class EEGEncoder(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.tct = TCT(config.eeg_input_dim, config.d_model, config.nhead, config.num_layers)
        self.gat = GATv2Layer(config.d_model, config.d_model)
    ...
    def forward(self, eeg_signal, eeg_spectrogram, adj):
        ...
        tct_out = self.tct(eeg_signal)                # (batch, time, d_model)
        # Pool over time dimension
        tct_out = tct_out.mean(dim=1)                 # (batch, d_model)
        # GAT expects (nodes, features). Assume adj is (channels, channels)
        gat_in = eeg_signal.permute(0,2,1)             # (batch, time, channels)
        gat_in = gat_in.mean(dim=1)                    # (batch, channels)
